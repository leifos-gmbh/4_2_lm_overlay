...
* @version $Id: class.ilObjFileGUI.php 50218 2014-05-21 10:02:47Z mjansen $
*
...
class ilObjFileGUI extends ilObject2GUI
{
	...
	*/
	function update()
	{
		...
		// endriss-patch: begin
		$this->object->enablePreDownloadHook($form->getInput('pre_download_hook'));
		// endriss-patch: end
		if (!empty($data["name"]))
		{
	...
	*/
	function edit()
	{
		...
		// endriss-patch: begin
		$val['pre_download_hook'] = $this->object->isPreDownloadHookEnabled();
		// endriss-patch: end
...
//		$file->enableFileNameSelection('title');
		...
		$form->addItem($desc);
		// endriss-patch: begin
		$chb = new ilCheckboxInputGUI($this->lng->txt('file_pre_download_hook'), 'pre_download_hook');
		$chb->setValue(1);
		$chb->setInfo($this->lng->txt('file_pre_download_hook_desc'));
		$form->addItem($chb);
		// endriss-patch: end
	...
	*/
	function versions()
	{
		...
		$hist_html = $hist_gui->getVersionsTable(
			...
			false, array($this, "getVersionsDownloadLink"));
		
		$this->tpl->setVariable("ADM_CONTENT", $hist_html);
	}
	
	function getVersionsDownloadLink($a_history_id)
	{
		global $ilCtrl;
		
		$ilCtrl->setParameter($this, "hist_id", $a_history_id);
		$link = $ilCtrl->getLinkTarget($this, "sendfile");
		$ilCtrl->setParameter($this, "hist_id", "");
