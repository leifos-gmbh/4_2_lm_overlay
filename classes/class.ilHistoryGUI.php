...
* @version $Id: class.ilHistoryGUI.php 33651 2012-03-09 16:51:30Z jluetzen $
*/
class ilHistoryGUI
{
	var $obj_id;
	...
	* get versions table
	*/
	function getVersionsTable($a_header_params, $a_user_comment = false, $a_callback = null)
	{
		$ref_id = $a_header_params["ref_id"];
		...
		if(count($entries) > 0)
		{
			...
			foreach($entries as $entry)
			{
				...
				if($a_callback)
				{
					$link = call_user_func($a_callback, $entry["hist_entry_id"]);
				}
				else
				{
					$link = "repository.php?cmd=sendfile&hist_id=".$entry["hist_entry_id"]."&ref_id=".$ref_id;
				}
