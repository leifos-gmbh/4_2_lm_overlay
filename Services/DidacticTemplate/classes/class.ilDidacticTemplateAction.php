...
abstract class ilDidacticTemplateAction
{
	...
	protected function filterRoles(ilObject $source)
	{
		...
		$filtered = array();
		foreach($rbacreview->getParentRoleIds($source->getRefId(),false,true) as $role_id => $role)
		{
			// no actions for system role id
			if($role_id == SYSTEM_ROLE_ID)
			{
				#$GLOBALS['ilLog']->write(__METHOD__.' no action for system role '.print_r($role,true));
				continue;
			}
