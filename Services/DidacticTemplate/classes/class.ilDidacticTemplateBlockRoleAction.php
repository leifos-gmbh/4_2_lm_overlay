...
class ilDidacticTemplateBlockRoleAction extends ilDidacticTemplateAction
{
	...
	public function  apply()
	{
		...
		{
			// Do not block protected roles
			if($role['protected'])
			{
				continue;
			}
			// Do not block local policies
			if($role['parent'] == $rolf)
			{
				continue;
			}
	...
	public function  revert()
	{
		...
		foreach($roles as $role_id => $role)
		{
			// Do not delete policies of local roles
			if($rbacreview->isLocalRole($role['obj_id'],$role['parent']))
			{
				continue;
			}

			$this->deleteLocalPolicy($role, $source);
		}
		return true;
	}
	 ...
	 * @param ilObject $source
	 */
	protected function deleteLocalPolicy($a_role, $source)
	{
		...
		$rbacadmin->deleteLocalRole($a_role['rol_id'], $rolf);

		// Change existing object
		include_once './Services/AccessControl/classes/class.ilObjRole.php';
		$role_obj = new ilObjRole($a_role['obj_id']);
		$role_obj->changeExistingObjects(
			$source->getRefId(),
			$a_role['protected'] ? ilObjRole::MODE_PROTECTED_DELETE_LOCAL_POLICIES : ilObjRole::MODE_UNPROTECTED_DELETE_LOCAL_POLICIES,
			array('all')
		);
		return true;
	}
